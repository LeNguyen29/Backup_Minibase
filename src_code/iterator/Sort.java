...
public class Sort extends Iterator implements GlobalConst {
  ...
  // task 4
  private int k; // num of top results to maintain
  private Vector100Dtype Target; // target vector for distance sorting
  private int returnedCount = 0; // counter of tuples returned
  ...
  public Sort(AttrType[] in,
      ...
      int k) throws IOException, SortException {
    ...
    order = sort_order;
    _n_pages = n_pages;
    this.Target = Target; // store target vector
    this.k = k; // store k value
    ...
    if (useBM) {
      ...
      // changed k to a for duplicate variable error
      for (int a = 0; a < _n_pages; a++)
        bufs[a] = new byte[MAX_SPACE];
    }
  ...
  public Tuple get_next()
    ...
    // if k is nonzero and already returned k tuples, stop
    if (k != 0 && returnedCount >= k) {
      return null;
    }
    ...
    if (output_tuple != null) {
      ...
      returnedCount++; // increment count of tuples returned
      return op_buf;
    } else
      return null;
  }
