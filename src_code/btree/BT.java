...
public class BT  implements GlobalConst{
  ...
  public final static int keyCompare(KeyClass key1, KeyClass key2)
	...
	return  (((IntegerKey)key1).getKey()).intValue() 
	  - (((IntegerKey)key2).getKey()).intValue();
        ...
        return ((Vector100DKey)key1).getHashValue() - ((Vector100DKey)key2).getHashValue();
      }
  ...
  protected final static int getKeyLength(KeyClass key) 
    ...
    if (key instanceof StringKey) {
        ...
        return 4; // Integer keys are 4 bytes
    } else if (key instanceof Vector100DKey) {
        return Vector100Dtype.SIZE * 2; // Each short is 2 bytes, and there are 100 elements
    } else {
   ...
   */
  protected final static KeyDataEntry getEntryFromBytes(byte[] from, int offset, int length, int keyType, short nodeType)
    ...
    try {
        ...
        if (keyType == AttrType.attrInteger) {
            ...
            key = new Vector100DKey(Convert.get100DVectorValue(offset, from));
        } else {
            throw new KeyNotMatchException(null, "key types do not match");
        }
  ...
  protected final static byte[] getBytesFromEntry(KeyDataEntry entry) 
    ...
    try {
        ...
        if (entry.key instanceof IntegerKey) {
            ...
            Convert.set100DVectorValue(((Vector100DKey) entry.key).getVector(), 0, data);
        } else {
            throw new KeyNotMatchException(null, "key types do not match");
        }
  ...
  public static void printPage(PageId pageno, int keyType)
    ...
    if (keyType == AttrType.attrVector100D)
      System.out.println(i + " (key, pageId):   (" +
             (Vector100DKey)entry.key + ",  " + (IndexData) entry.data + " )");
