...
public class RSIndexScan extends Iterator
{
   ...
   * @param distance non-negative integer indicating the range (distance) for the scan
   ...
   */
    ...
    UnknownIndexTypeException, IOException
    {
        ...
        // Set up the output tuple types based on the input types and the fields to project
        Jtuple = new Tuple();
        try {
            ts_sizes = TupleUtils.setup_op_tuple(Jtuple, Jtypes, types, noInFlds, str_sizes, outFlds, noOutFlds);
        }
        catch (TupleUtilsException e) {
            throw new IndexException(e, "RSIndexScan.java: TupleUtilsException caught from TupleUtils.setup_op_tuple()");
        }
